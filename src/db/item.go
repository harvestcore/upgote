...
// 	res, err := item.Collection().InsertMany(Ctx(), elements)
...
func (item *Item) Update(element map[string]interface{}, updated map[string]interface{}) UpdateResponse {
	res, err := item.Collection().UpdateOne(Ctx(), element, updated)
...
func (item *Item) Delete(element map[string]interface{}) DeleteResponse {
	res, err := item.Collection().DeleteOne(Ctx(), element)
	deleted := 0
	status := false
